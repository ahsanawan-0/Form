...
const DateField = ({ startDate, setStartDate, endDate, setEndDate }) => {
  ...
  const handleStartDateChange = (date) => {
    ...
    const utcDate = moment(date).utc().startOf("day").toDate();
    setStartDate(utcDate);
    const updatedErrors = validateDates(utcDate, endDate);
  ...
  const handleEndDateChange = (date) => {
    ...
    const utcDate = moment(date).utc().startOf("day").toDate();
    setEndDate(utcDate);
    const updatedErrors = validateDates(startDate, utcDate);
    setErrors(updatedErrors);
  };
  ...
  const handleDateInputChange = (event, setDate, setValidity) => {
    ...
    if (isValidDate) {
      const date = moment
        .tz(inputValue, "MM-DD-YYYY", userTimeZone)
        .startOf("day")
        .toDate();
      setDate(date);
      ...
      >
        ...
        <div className="relative">
          <DatePicker
            ...
            onChangeRaw={(e) =>
              handleDateInputChange(e, setStartDate, setIsDateStartedValid)
            }
            startDate={startDate}
            endDate={endDate}
            selectsStart
            ...
            className={`w-[100%]  px-4 py-2 border placeholder-blue-400 uppercase ${
          ...
          />
          <TbCalendarCheck
            className="absolute top-1/2 right-10   flex items-center justify-center text-xl transform -translate-y-1/2 text-black cursor-pointer"
        ...
        {!isDateStartedValid && (
          <div className="mt-1 text-red-600 text-sm flex items-center">
            ...
            <span>
              ...
              format is MM-DD-YYYY, and the date is not in the past.
            </span>
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className="relative">
          <DatePicker
            ...
            onChangeRaw={(e) =>
              handleDateInputChange(e, setEndDate, setIsDateExitedValid)
            }
            startDate={startDate}
            endDate={endDate}
            selectsEnd
        ...
        {!isDateExitedValid && (
          <div className="mt-1 text-red-600 text-sm flex items-center">
            <AiOutlineWarning className="mr-1" />
            <span>
              This date is not valid. Ensure the date is after the DateStarted and in the correct format.
            </span>
          </div>
        )}
      </div>
